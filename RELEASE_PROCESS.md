...
Der Beta-Stand ist `1.1.9-pre.4` (Version Code 22), mit Beobachtungsfenster heute Nacht („Heute Nacht im Überblick“), ausrüstungsbasierten Empfehlungen („Was lohnt sich heute Nacht?“), Qualitäts-Scores (0-100), lokalem Standortspeicher (LocationStore) und priorisiertem Basiskartenstart vor GPU/Texturladen. Dieser Pre-Release wird von `beta` erstellt; `main` bleibt unverändert bei `1.1.8-pre.1` (Version Code 18). Ein Pre-Release ist kein Play-Store-Produktivstand; Datenschutz-, Signatur- und Store-Prüfungen bleiben vor der stabilen Veröffentlichung erforderlich.
