...
Der Beta-Stand ist `1.1.9-pre.10` (Version Code 28), mit der Akku- und Energieoptimierung (P3): Lifecycle-gesteuerte Sensorderegistrierung in `rememberOrientation` zur vollständigen Vermeidung von Standby-Aktivität nach Bildschirm-Aus, Energiemessmatrix in `play-store/battery-profiling.md` und automatisierte Dormancy-Zusicherungen in `SkyStartupTest.kt`. Dieser Pre-Release wird von `beta` erstellt; `main` bleibt unverändert bei `1.1.8-pre.1` (Version Code 18). Ein Pre-Release ist kein Play-Store-Produktivstand; Datenschutz-, Signatur- und Store-Prüfungen bleiben vor der stabilen Veröffentlichung erforderlich.
